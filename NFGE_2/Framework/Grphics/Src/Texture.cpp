...
void Texture::Resize(uint32_t width, uint32_t height, uint32_t depthOrArraySize)
{
    ...
    if (mD3d12Resource)
    {
        ResourceStateTracker::RemoveGlobalResourceState(mD3d12Resource.Get());

        CD3DX12_RESOURCE_DESC resDesc(mD3d12Resource->GetDesc());
...
{
    auto device = NFGE::Graphics::GetDevice();
    auto srv = NFGE::Graphics::AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

    device->CreateShaderResourceView(mD3d12Resource.Get(), srvDesc, srv.GetDescriptorHandle());
...
{
    auto device = NFGE::Graphics::GetDevice();
    auto uav = NFGE::Graphics::AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

    device->CreateUnorderedAccessView(mD3d12Resource.Get(), nullptr, uavDesc, uav.GetDescriptorHandle());
