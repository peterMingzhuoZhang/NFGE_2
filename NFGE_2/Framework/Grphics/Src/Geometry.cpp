...
void NFGE::Graphics::GeometryPNX::Load(MeshPNX mesh, DirectionalLight* directionLight)
{
	mMesh = std::move(mesh);
	...
	NFGE::Graphics::GraphicsSystem::UpdateBufferResource(commandList.Get(),
		...
		vertices.size(), sizeof(VertexPNX), vertices.data());
	...
	mMeshRenderStrcuture.vertexBufferView.StrideInBytes = sizeof(VertexPNX);
	...
	auto vertexLayout = NFGE::Graphics::GetVectexLayout(VertexPNX::Format);
...
void NFGE::Graphics::GeometryPNX::UnLoad()
{
}

void NFGE::Graphics::GeometryPNX::Update(float deltaTime)
{
}

void NFGE::Graphics::GeometryPNX::Render(const NFGE::Graphics::Camera& camera)
{
	...
	commandList->DrawIndexedInstanced(mMesh.GetIndices().size(), 1, 0, 0, 0);
